...
// TestParseReadings verifies CSV parsing, the malformed-row error path and the
// missing-file error path.
func TestParseReadings(t *testing.T) {
	// Malformed row: non-numeric irradiance must error.
...
// TestByInverter verifies grouping by inverter and nil-safety.
func TestByInverter(t *testing.T) {
	// nil-safe: returns a non-nil empty map.
